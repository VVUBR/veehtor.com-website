// WebGL Orb background component
...
export default function Orb({
  ...
  if (reduce) {
    return (
      <div
        ...
        style={{
          background:
            'radial-gradient(80% 80% at 50% 50%, rgba(6,182,212,0.25), rgba(6,182,212,0) 60%), radial-gradient(60% 60% at 70% 30%, rgba(99,102,241,0.25), rgba(99,102,241,0) 60%)',
          filter: 'blur(18px)'
        }}
      />
    );
  ...
  const frag = /* glsl */ `
    ...
    vec3 adjustHue(vec3 col,float h){float a=radians(h);vec3 y=rgb2yiq(col);float ca=cos(a);float sa=sin(a);float I=y.y*ca - y.z*sa;float Q=y.y*sa + y.z*ca;y.y=I;y.z=Q;return yiq2rgb(y);}
    ...
    const float innerR=0.6;const float ns=0.65;
    float l1(float it,float at,float d){return it/(1.0+d*at);}
    float l2(float it,float at,float d){return it/(1.0+d*d*at);}
    vec4 draw(vec2 uv){
  ...
  useEffect(() => {
    ...
    // Try WebGL context; bail gracefully if not supported
    ...
    const handleLeave = () => { targetHover = 0; };

    const targetEl = capture === 'container' ? container : window;
    targetEl.addEventListener('mousemove', handleMouseMove as any);
    ...
    return () => {
      ...
      targetEl.removeEventListener('mousemove', handleMouseMove as any);
      window.removeEventListener('mouseleave', handleLeave);
      try { container.removeChild(gl.canvas); } catch {}
      gl.getExtension('WEBGL_lose_context')?.loseContext();
    };
  }, [hue, hoverIntensity, rotateOnHover, forceHoverState, capture]);

  return <div ref={ctnDom} className={`w-full h-full pointer-events-none ${className}`} aria-hidden />;
}
